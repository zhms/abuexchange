...
func MarketTicker() {
	for i := 0; i < len(reptile.Symbols); i++ {
		go market_ticker(reptile.Symbols[i])
	}
}

func market_ticker(symbol string) {
	defer recover()
	conn, _, err := websocket.DefaultDialer.Dial(reptile.MarketStreamUrl, nil)
	if err != nil {
		fmt.Println("market ticker:", err)
		time.Sleep(time.Second * 1)
		go market_ticker(symbol)
		return
	}
	...
	for {
		...
		go market_ticker_message(message)
	}
	time.Sleep(time.Second * 1)
	go market_ticker(symbol)
}
...
func market_ticker_message(s []byte) {
	defer recover()
	...
	rediskey := fmt.Sprint("reptile:market:ticker:", symbol)
	redisconn := reptile.RedisPool.Get()
	redisconn.Do("set", rediskey, strdata)
	redisconn.Do("publish", "reptile_market_ticker", fmt.Sprint(symbol, "@", string(strdata)))
	redisconn.Close()
}
