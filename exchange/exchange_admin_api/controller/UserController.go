...
func user_list(ctx *abugo.AbuHttpContent) {
	...
	var total int
	server.Db().QueryScan(where.CountSql("ex_user"), where.Params, &total)
	if total == 0 {
	...
	dbresult, err := server.Db().Conn().Query(where.Sql("ex_user", reqdata.Page, reqdata.PageSize), where.GetParams()...)
	if err != nil {
		logs.Error(err)
